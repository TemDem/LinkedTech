...
!(function (t, e) {
	...
	const storeHouseGrid = document.querySelector(".storehouse-grid")
const msnry = new Masonry(storeHouseGrid, {
	itemSelector: ".storehouse-item",
});
		...
		zooms.forEach(el => {
			if (e.target === el) {
				if (el.classList.contains("_icon-zoom-plus")) {
					const parentCartChildes = [...el.closest("._item-wrapper").children]
					parentCartChildes.forEach(el => {
						if (el.classList.contains("_item-image")) {
							el.classList.add("_active")
						}
					})
